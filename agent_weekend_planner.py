...
def generate_itineraries(weather, calendar_events, saturday_date, sunday_date):
    ...
    **TIME & SCHEDULE RULES:**
    - Drive time: Max 90 minutes from Westfield, NJ.
    - Buffer time: Always add 30 mins after sports/swim lessons for changing/prep before driving or eating.
    ...
    **PREFERENCES:**
    ...
    - Parents' Preferences: Healthy options, farm-to-table, breweries, unique local spots.
    - Dog-Friendly: Explicitly flag whether the activity is 🐶 Dog Friendly or 🚫 No Dogs Allowed.
    - Restaurants: Secondary to the activity. Do NOT plan a day around a restaurant. Only include a restaurant if it's within a 30-minute drive of the activity or on the way home, and offers unique/farm-to-table/brewery vibes. Otherwise omit.
    ...
    **DELIVERABLE:**
    ...
    7. Full Timeline (accounting for travel from Westfield, calendar events, buffers, and meals)
    8. Recommended Nearby Restaurant (Food type, recommended dishes, distance from activity) - ONLY if exceptional and within 30 mins.
    
    Return ONLY valid HTML inside `<div>` tags with inline CSS styling suitable for an email digest. Do NOT wrap in markdown ```html codeblocks.
    """
...
def send_email(html_content):
    ...
    msg["Subject"] = "🗓️ Your Westfield Weekend Family Itinerary Digest"
    msg["From"] = GMAIL_USER
    msg["To"] = RECIPIENT_EMAIL
    msg["Reply-To"] = RECIPIENT_EMAIL  # Allows direct email replies for feedback
    ...
    <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.5; max-width: 700px; margin: auto;">
        ...
        <p><i>💡 <b>Feedback Loop:</b> Simply reply to this email with what you liked, picked, or skipped to help refine future suggestions!</i></p>
    ...
    try:
        ...
        server.quit()
        print("Weekend Planner digest dispatched successfully via Gmail SMTP!")
    except Exception as e:
